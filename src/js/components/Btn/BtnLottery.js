...
class BtnLottery extends React.Component {
	...
	lottery() {
		var that = this
		var n = document.querySelector(".hidden").innerHTML - 0;
		n--
		var data = this.state.data
		new MyAjax({
			...
			callback(data) {
				...
				data.code == "2003" ? document.querySelector('.get-now').style.display = "none" : ''
	...
	componentWillMount() {
		...
		new MyAjax({
			...
			method: "POST",
			data: "intfId=15065001688055506530264090806290&isDefault=0",
			callback(data) {
	...
	componentDidMount() {
		...
		document.querySelector(".frequency").innerHTML = document.querySelector(".hidden").innerHTML
		img.onload = () => {
